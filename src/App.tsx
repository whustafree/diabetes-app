...
export default function App() {
  ...
  if (loading && firebaseReady) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-slate-900 flex items-center justify-center transition-colors duration-300">
        <div className="text-center">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center mx-auto mb-4 shadow-2xl shadow-blue-200 dark:shadow-blue-900/50">
            <Activity className="w-8 h-8 text-white" />
          </div>
          <Loader2 className="w-8 h-8 text-blue-500 mx-auto animate-spin" />
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-3">Cargando...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-slate-900 transition-colors duration-300">
      {/* Top Navigation Bar */}
      <header className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg border-b border-gray-100 dark:border-gray-700 sticky top-0 z-50 transition-colors">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-2.5 flex-shrink-0">
              ...
              <div className="hidden sm:block">
                <h1 className="text-base font-extrabold text-gray-800 dark:text-white leading-tight">Diabetes Control</h1>
                <p className="text-[10px] text-gray-400 dark:text-gray-500 leading-tight">Salud y Bienestar</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {!isOnline && (
                <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-[11px] font-semibold bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 transition-all duration-300" title="Sin conexión a internet">
                  <CloudOff className="w-3.5 h-3.5" />
                  <span>Sin conexión</span>
                  {pendingCount > 0 && (
                    <span className="ml-1 px-1.5 py-0.5 rounded-md bg-red-200 dark:bg-red-800 text-[9px] font-bold">
              ...
              <div className="relative">
                <button
                  ...
                  className="p-2 rounded-xl text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-all relative"
                  title="Notificaciones"
                >
                  <BellRing className="w-5 h-5" />
                  {unreadCount > 0 && (
                    <span className="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center shadow-lg shadow-red-200 dark:shadow-red-900/50 animate-pulse">
                ...
                {showNotificationPanel && (
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 z-50 overflow-hidden animate-[fadeIn_0.2s_ease-out]">
                      <div className="p-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                        <h4 className="text-sm font-bold text-gray-800 dark:text-white">Notificaciones</h4>
                        {notificationsHistory.length > 0 && (
                          <button onClick={markAllRead} className="text-[11px] text-blue-500 font-semibold hover:text-blue-600 transition">
                            Marcar todas leídas
                          </button>
                        )}
                      </div>
                      <div className="max-h-64 overflow-y-auto">
                        {notificationsHistory.length === 0 ? (
                          <div className="py-8 text-center">
                            <BellRing className="w-8 h-8 text-gray-200 dark:text-gray-600 mx-auto mb-2" />
                            <p className="text-xs text-gray-400 dark:text-gray-500">Sin notificaciones</p>
                          </div>
                        ) : (
                          notificationsHistory.map((n, i) => (
                            ...
                            >
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                {n.body && <p className="text-[11px] text-gray-500 dark:text-gray-400 truncate mt-0.5">{n.body}</p>}
              ...
              {firebaseReady && user && (
                <div className="relative" ref={userMenuRef}>
                  <button
                    ...
                    className="flex items-center gap-1.5 p-1.5 pr-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition-all group"
                  ...
                  >
                    {user.photoURL ? (
                      <img
                        src={user.photoURL}
                        alt="Foto de perfil"
                        className="w-8 h-8 rounded-xl object-cover border-2 border-gray-200 dark:border-gray-600 group-hover:border-blue-400 transition-colors"
                    ...
                    <span className="hidden sm:block text-xs text-gray-500 dark:text-gray-400 font-medium truncate max-w-[100px]">
                  ...
                  {/* Dropdown menu */}
                  {showUserMenu && (
                    <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 z-[60] overflow-hidden animate-[fadeIn_0.15s_ease-out]">
                      {/* User info header */}
                      ...
                      <div className="p-2">
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all"
                        >
              ...
              {/* Navigation — Desktop */}
              <nav className="hidden md:flex items-center gap-1 bg-gray-100/80 dark:bg-gray-700/80 rounded-2xl p-1">
                {navItems.map(item => {
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-gray-800/90 backdrop-blur-lg border-t border-gray-100 dark:border-gray-700 z-50" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
          ...
          {navItems.map(item => {
            ...
            return (
              ...
              >
                ...
                {isActive && <div className="w-4 h-0.5 rounded-full bg-blue-600 dark:bg-blue-400 mt-0.5" />}
              </button>
            );
          })}
        </div>
...
const notifTypeStyles: Record<string, { icon: string; bg: string; color: string }> = {
  glucose: { icon: '🩸', bg: 'bg-blue-100 dark:bg-blue-900/40', color: 'text-blue-600 dark:text-blue-400' },
  medication: { icon: '💊', bg: 'bg-red-100 dark:bg-red-900/40', color: 'text-red-600 dark:text-red-400' },
  meal: { icon: '🍽️', bg: 'bg-green-100 dark:bg-green-900/40', color: 'text-green-600 dark:text-green-400' },
  exercise: { icon: '🏃', bg: 'bg-orange-100 dark:bg-orange-900/40', color: 'text-orange-600 dark:text-orange-400' },
  water: { icon: '💧', bg: 'bg-cyan-100 dark:bg-cyan-900/40', color: 'text-cyan-600 dark:text-cyan-400' },
  custom: { icon: '⏰', bg: 'bg-purple-100 dark:bg-purple-900/40', color: 'text-purple-600 dark:text-purple-400' },
};
...
function ToastNotification({ id, title, body, type, onDismiss }: {
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 p-4 flex items-start gap-3 relative overflow-hidden">
        {/* Barra de tipo */}
        <div className={`absolute top-0 left-0 w-1 h-full rounded-l-2xl ${style.bg.replace('dark:', '').replace('/40', '/60')}`} />
        ...
        <button
          onClick={onDismiss}
          className="p-1.5 text-gray-300 dark:text-gray-500 hover:text-gray-500 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-xl transition flex-shrink-0 self-start"
        >
          <X className="w-4 h-4" />
        </button>
...
function PWAInstallPrompt() {
  ...
  return (
    <div className="fixed bottom-4 right-4 left-4 sm:left-auto sm:w-80 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 p-4 z-50 animate-[fadeIn_0.3s_ease-out]">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-gray-800 dark:text-white">Instala Diabetes Control</p>
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Agrega la app a tu pantalla de inicio</p>
        </div>
        <button
        ...
        <button
          ...
          className="p-1 text-gray-300 dark:text-gray-500 hover:text-gray-500 dark:hover:text-gray-300 transition flex-shrink-0"
        >
          ✕
        </button>
      </div>
