...
const notifTypeMeta: Record<string, { icon: string; label: string; color: string }> = {
 glucose: { icon: '🩸', label: 'Glucosa', color: 'bg-blue-100 bg-blue-900/40 text-blue-400' },
 medication: { icon: '💊', label: 'Medicación', color: 'bg-red-100 bg-red-900/40 text-red-400' },
 meal: { icon: '🍽️', label: 'Comida', color: 'bg-green-100 bg-green-900/40 text-green-400' },
 exercise: { icon: '🏃', label: 'Ejercicio', color: 'bg-orange-100 bg-orange-900/40 text-orange-400' },
 water: { icon: '💧', label: 'Agua', color: 'bg-cyan-100 bg-cyan-900/40 text-cyan-400' },
 custom: { icon: '⏰', label: 'Personalizado', color: 'bg-purple-100 bg-purple-900/40 text-purple-400' },
};
...
export default function NotificationsPage() {
 ...
 <span className="text-xs font-semibold text-gray-400 text-gray-500">No leídas</span>
 </div>
 ...
 <span className="text-xs font-semibold text-gray-400 text-gray-500">Push</span>
 </div>
 ...
 <span className="text-xs font-semibold text-gray-400 text-gray-500">Tipos</span>
 </div>
 ...
 <span className="text-xs font-semibold text-gray-400 text-gray-500">Hoy</span>
 </div>
 ...
 <div className="bg-gray-800 rounded-2xl p-5 border border-gray-700 shadow-sm space-y-4">
 ...
 <span className="text-[9px] font-medium text-gray-400 text-gray-500">{day.count}</span>
 <div
 ...
 />
 <span className={`text-[9px] font-medium ${isToday ? 'text-blue-500 font-bold' : 'text-gray-400 text-gray-500'}`}>
 {day.day}
 </span>
 </div>
 );
 ...
 <p className="text-[10px] text-gray-400 text-gray-500">Día más activo ({weeklyStats.busiestDay.count})</p>
 </div>
 </div>
 ...
 className="p-2.5 rounded-xl bg-red-900/30 text-red-400 hover:text-red-600 hover:bg-red-100 hover:bg-red-900/50 transition-all"
 title="Limpiar todo"
 >
 ...
 {/* Type filters */}
 {showFilters && (
 <div className="flex flex-wrap gap-1.5 bg-gray-800 rounded-2xl p-3 border border-gray-700">
 ...
 ? 'bg-blue-100 bg-blue-900/50 text-blue-300 ring-1 ring-blue-300'
 : 'bg-gray-50 bg-gray-700 text-gray-400 hover:bg-gray-700 hover:bg-gray-600'
 }`}
 >
 {opt.icon} {opt.label}
 </button>
 ...
 : 'bg-blue-50/80 bg-blue-900/15 border-blue-700 border-blue-800'
 }`}
 ...
 <div className="flex items-center gap-2">
 <p className={`text-sm font-semibold ${n.read ? 'text-gray-600 text-gray-300' : 'text-gray-800 text-white'}`}>
 {n.title}
 </p>
 {!n.read && (
 ...
 {n.body && (
 <p className={`text-xs mt-0.5 line-clamp-1 ${n.read ? 'text-gray-400 text-gray-500' : 'text-gray-400'}`}>
 {n.body}
 </p>
 )}
 <div className="flex items-center gap-2 mt-1">
 <Clock className="w-3 h-3 text-gray-300 text-gray-600"/>
 <span className="text-[10px] text-gray-300 text-gray-600">{timeStr}</span>
 {n.type && (
 ...
 className="p-1 text-gray-300 text-gray-400 hover:text-gray-500 hover:text-gray-300 hover:bg-gray-700 rounded-lg transition flex-shrink-0 self-start"
 >
 <X className="w-4 h-4"/>
 </button>
 </div>
 ...
 {/* Footer info */}
 <div className="text-center">
 <p className="text-[11px] text-gray-300 text-gray-600">
 ...
 <div className="bg-gray-800 rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden animate-[scaleIn_0.2s_ease-out]"onClick={e => e.stopPropagation()}>
 ...
 {!selectedNotif.read && (
 <span className="text-xs font-semibold text-blue-400 bg-blue-900/30 px-2 py-1 rounded-full">Nueva</span>
 )}
 </div>
 <button
 onClick={() => setSelectedNotif(null)}
 className="p-1.5 text-gray-300 text-gray-400 hover:text-gray-500 hover:text-gray-300 hover:bg-gray-700 rounded-xl transition"
 >
 <X className="w-5 h-5"/>
 </button>
 </div>
 ...
 {/* Metadata */}
 <div className="bg-gray-50 bg-gray-700/50 rounded-2xl p-4 space-y-3">
 <div className="flex items-center justify-between text-sm">
 <span className="text-gray-400 text-gray-500">Fecha y hora</span>
 ...
 <div className="flex items-center justify-between text-sm">
 <span className="text-gray-400 text-gray-500">Tipo</span>
 <span className="font-semibold text-gray-300 text-gray-200">
 {(notifTypeMeta[selectedNotif.type || 'custom'] || notifTypeMeta.custom).label}
 </span>
 </div>
 <div className="flex items-center justify-between text-sm">
 <span className="text-gray-400 text-gray-500">Estado</span>
 <span className={`font-semibold ${selectedNotif.read ? 'text-green-500' : 'text-blue-500'}`}>
 {selectedNotif.read ? 'Leída' : 'No leída'}
 </span>
 </div>
 <div className="flex items-center justify-between text-sm">
 <span className="text-gray-400 text-gray-500">ID</span>
 <span className="text-xs font-mono text-gray-400 text-gray-400 truncate max-w-[200px]">{selectedNotif.id}</span>
 </div>
 </div>
 ...
 className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-red-900/30 text-red-400 font-semibold hover:bg-red-100 hover:bg-red-900/50 transition-all active:scale-[0.98]"
 >
 <Trash2 className="w-4 h-4"/>
 Eliminar
 </button>
