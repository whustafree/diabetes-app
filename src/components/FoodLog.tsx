    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h2 className="text-2xl font-extrabold text-gray-800 dark:text-white">Registro de Comidas</h2>
          </div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-red-500 text-white font-semibold hover:from-orange-600 hover:to-red-600 shadow-lg shadow-orange-200 dark:shadow-orange-900/50 transition-all active:scale-95"
        >
      ...
      {weekEntries.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <p className="text-2xl font-extrabold text-gray-800 dark:text-white mt-1">{stats.totalMeals}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Carbohidratos totales</p>
            <p className="text-2xl font-extrabold text-orange-500 mt-1">{stats.totalCarbs}g</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Promedio por comida</p>
            <p className="text-2xl font-extrabold text-gray-800 dark:text-white mt-1">{stats.averageCarbsPerMeal}g</p>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 rounded-2xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 dark:text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-200 outline-none transition-all text-sm"
        />
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-12 shadow-sm border border-gray-100 dark:border-gray-700 text-center">
          <UtensilsCrossed className="w-12 h-12 text-gray-200 dark:text-gray-600 mx-auto mb-3" />
...
function FoodForm({ onSubmit, onCancel }: {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-md border border-orange-100 dark:border-gray-700 space-y-4">
      {/* Meal type selector */}
        ...
        {mealTypes.map(type => (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all ${
              mealType === type
                ? 'bg-orange-100 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-700'
                : 'bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border border-gray-100 dark:border-gray-600 hover:border-orange-200'
            }`}
          >
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 dark:text-white focus:bg-white dark:focus:bg-gray-700 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 outline-none transition-all text-sm resize-none"
          required
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            placeholder="45"
            className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 dark:text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-200 outline-none transition-all text-sm"
          />
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                placeholder="120"
                className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 dark:text-white focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all text-sm"
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1 flex items-center gap-1">
      ...
      <div className="flex items-center justify-end gap-3 pt-2">
        <button
          type="button"
          onClick={onCancel}
          className="px-4 py-2.5 rounded-xl text-sm font-semibold text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all"
        >
          Cancelar
        </button>
        <button
          type="submit"
          className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-red-500 text-white font-semibold hover:from-orange-600 hover:to-red-600 transition-all active:scale-95 text-sm shadow-lg shadow-orange-200 dark:shadow-orange-900/50"
        >
          Guardar registro
        </button>
      </div>
...
function FoodLogCard({ entry, onDelete }: { entry: FoodLogEntry; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-all">
      <div className="flex items-start gap-3">
        {/* Icon */}
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0 ${
          entry.mealType === 'desayuno' ? 'bg-yellow-100 dark:bg-yellow-900/30' :
          entry.mealType === 'almuerzo' ? 'bg-orange-100 dark:bg-orange-900/30' :
          entry.mealType === 'cena' ? 'bg-indigo-100 dark:bg-indigo-900/30' :
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-xs font-semibold text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-lg">
          ...
          <div className="flex items-center gap-3 mt-2">
            {entry.carbs !== undefined && (
              <span className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400 bg-orange-50 dark:bg-orange-900/20 px-2 py-0.5 rounded-lg">
                <Wheat className="w-3 h-3 text-orange-500" />
                {entry.carbs}g carbs
              </span>
            )}
            {entry.calories !== undefined && (
              <span className="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400 bg-green-50 dark:bg-green-900/20 px-2 py-0.5 rounded-lg">
                <Apple className="w-3 h-3 text-green-500" />
                {entry.calories} kcal
              </span>
            )}
        ...
        <div className="relative flex-shrink-0">
          ...
          {showDelete && (
            <div className="absolute right-0 top-full mt-1 bg-white dark:bg-gray-700 rounded-xl shadow-xl border border-gray-100 dark:border-gray-600 z-10 p-2 min-w-[140px] animate-[fadeIn_0.15s_ease-out]">
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-2 px-2">¿Eliminar registro?</p>
              <div className="flex gap-1">
                <button
                  onClick={() => setShowDelete(false)}
                  className="flex-1 px-3 py-1.5 rounded-lg text-xs font-semibold text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-600 transition-all"
                >
                  No
                </button>
                <button
