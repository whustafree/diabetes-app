  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {entries.length > 0 && liveGlucose !== null && liveStatus && (
        <div className="card-enter bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden relative">
          {/* Live indicator bar */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-baseline gap-2 mt-1">
                  ...
                  <span className="text-sm font-medium text-gray-400 dark:text-gray-500">mg/dL</span>
            ...
            <div className="text-right flex-shrink-0">
              <div className="flex items-center gap-2">
                <svg className={`w-4 h-4 ${countdown < 60 ? 'text-orange-400 animate-spin' : 'text-gray-300 dark:text-gray-600'}`} style={{ animationDuration: countdown < 60 ? '2s' : '3s' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <circle cx="12" cy="12" r="10" />
                  <path d="M12 6v6l4 2" />
                </svg>
                <span className={`text-xs font-mono font-bold ${countdown < 60 ? 'text-orange-400' : 'text-gray-400 dark:text-gray-500'}`}>
      ...
      {/* ─── PERSONALIZED HEADER ─── */}
      <div className="card-enter bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 rounded-3xl p-6 sm:p-8 text-white shadow-xl shadow-blue-200 dark:shadow-blue-900/50">
      ...
      {/* ─── NO PROFILE ─── */}
      {!profile && (
        <div className="card-enter bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-2xl p-6 border border-blue-100 dark:border-blue-800/50 text-center">
          ...
          <button
            onClick={() => onNavigate?.('profile')}
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-200 dark:shadow-blue-900/50 transition-all active:scale-95"
          >
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="card-enter card-hover bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="flex items-center justify-between mb-3">
            ...
            {medications.length > 0 && (
              <button onClick={() => onNavigate?.('medications')} className="text-[10px] font-semibold text-emerald-500 hover:text-emerald-600 transition-colors">
                Ver todas
              </button>
            )}
          </div>
          ...
          {medications.length === 0 ? (
            <div className="text-center py-4">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500">No hay medicamentos registrados</p>
              <button
                onClick={() => onNavigate?.('medications')}
                className="text-xs text-emerald-500 font-semibold hover:underline mt-1 inline-block"
              >
            ...
            <div className="flex items-center gap-3 py-3">
              ...
              <div>
                <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Al día</p>
                <p className="text-xs text-gray-400 dark:text-gray-500">Todas las dosis están al día</p>
              </div>
            </div>
          ) : (
              ...
              {upcomingMeds.map(({ med, hours, minutes, isDue, nextTime }) => (
                <div key={med.id} className={`flex items-center gap-3 p-2.5 rounded-xl transition-all ${
                  ...
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                    isDue ? 'bg-red-100 dark:bg-red-900/50' : 'bg-emerald-100 dark:bg-emerald-900/50'
                  }`}>
                    {isDue ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-gray-700 dark:text-gray-200 truncate">{med.name}</p>
                    <p className="text-[10px] text-gray-400 dark:text-gray-500">{med.dosage}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    {isDue ? (
                      ...
                      <>
                        ...
                        <p className="text-[10px] text-gray-400 dark:text-gray-500">{new Date(nextTime).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</p>
                      </>
                    )}
                  </div>
                </div>
        ...
        <div className="card-enter card-hover bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
          <div className="flex items-center justify-between mb-3">
            ...
            {reminders.length > 0 && (
              <button onClick={() => onNavigate?.('reminders')} className="text-[10px] font-semibold text-purple-500 hover:text-purple-600 transition-colors">
                Ver todos
              </button>
            )}
          </div>
          ...
          {todayReminders.length === 0 ? (
            <div className="text-center py-4">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500">No hay recordatorios para hoy</p>
              <button
                onClick={() => onNavigate?.('reminders')}
                className="text-xs text-purple-500 font-semibold hover:underline mt-1 inline-block"
              >
                Crear recordatorios
              </button>
            </div>
            ...
            <div className="space-y-2">
              {todayReminders.slice(0, 4).map(reminder => {
                ...
                return (
                  <div key={reminder.id} className={`flex items-center gap-3 p-2.5 rounded-xl ${
                    isPast ? 'bg-gray-50 dark:bg-gray-700/50 opacity-60' : 'bg-purple-50 dark:bg-purple-900/20'
                  }`}>
                    <span className="text-lg">{reminderTypeIcons[reminder.type]}</span>
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs font-bold truncate ${isPast ? 'text-gray-400 dark:text-gray-500' : 'text-gray-700 dark:text-gray-200'}`}>
                        {reminder.title}
                      </p>
                      <p className={`text-[10px] ${isPast ? 'text-gray-300 dark:text-gray-600' : 'text-gray-400 dark:text-gray-500'}`}>
                        {reminder.description || reminderTypeLabels[reminder.type]}
                      </p>
                    </div>
                    <span className={`text-[10px] font-semibold flex-shrink-0 ${
                      isPast ? 'text-gray-300 dark:text-gray-600' : 'text-purple-500'
                    }`}>
                      {reminder.time}
                    </span>
                  </div>
              ...
              {todayReminders.length > 4 && (
                <button
                  onClick={() => onNavigate?.('reminders')}
                  className="w-full text-center text-[10px] font-semibold text-purple-500 hover:text-purple-600 py-1 transition-colors"
                >
      ...
      {/* ─── GLUCOSE SECTION ─── */}
      <div className="card-enter bg-white dark:bg-gray-800 rounded-2xl p-5 sm:p-6 shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        {/* Tab Navigation */}
        <div className="flex gap-1 bg-gray-100 dark:bg-gray-800/50 rounded-2xl p-1.5 mt-4">
          {[
        ...
        <div className="card-enter card-hover bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-3">
              <p className="text-[10px] font-semibold text-blue-500 uppercase tracking-wider">Peso ideal</p>
            ...
            <div className="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-3">
              <p className="text-[10px] font-semibold text-purple-500 uppercase tracking-wider">Grasa saludable</p>
            ...
            <div className="bg-orange-50 dark:bg-orange-900/20 rounded-xl p-3">
              <p className="text-[10px] font-semibold text-orange-500 uppercase tracking-wider">Calorías diarias</p>
            ...
            <div className={`rounded-xl p-3 ${
              assessment.riskLevel === 'bajo' ? 'bg-green-50 dark:bg-green-900/20' :
              assessment.riskLevel === 'moderado' ? 'bg-yellow-50 dark:bg-yellow-900/20' :
              ...
              <p className={`text-sm font-bold mt-1 ${
                assessment.riskLevel === 'bajo' ? 'text-green-700 dark:text-green-300' :
                assessment.riskLevel === 'moderado' ? 'text-yellow-700 dark:text-yellow-300' :
...
function MonthlyComparisonCard({ entries }: { entries: GlucoseEntry[] }) {
  ...
  return (
    <div className="card-hover bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-bold text-gray-800 dark:text-white flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <div className="text-center">
          <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Promedio</p>
        </div>
        <div className="text-center">
          <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">En rango</p>
        </div>
        <div className="text-center">
          <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Mediciones</p>
        </div>
        <div className="text-center">
          <p className="text-[10px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Rango</p>
        </div>

        {/* Previous month */}
        <div className="flex items-center gap-2">
          <div className="w-2.5 h-2.5 rounded-sm bg-gray-300 dark:bg-gray-600 flex-shrink-0" />
          <span className="text-xs font-bold text-gray-500 dark:text-gray-400">{monthNames[prevMonth]}</span>
        </div>
        <div className="text-center">
        ...
        <div className="text-center">
          <p className={`text-sm font-bold ${previous.inRangePct >= 70 ? 'text-green-600 dark:text-green-400' : previous.inRangePct >= 40 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'}`}>
            {previous.readings > 0 ? `${previous.inRangePct}%` : '—'}
          </p>
        </div>
        <div className="text-center">
          <p className="text-sm font-bold text-gray-700 dark:text-gray-200">{previous.readings}</p>
        </div>
        <div className="text-center">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400">{monthNames[currentMonth]}</span>
        </div>
        <div className="text-center">
        ...
        <div className="text-center">
          <p className={`text-sm font-bold ${current.inRangePct >= 70 ? 'text-green-600 dark:text-green-400' : current.inRangePct >= 40 ? 'text-yellow-600 dark:text-yellow-400' : 'text-red-600 dark:text-red-400'}`}>
            {current.readings > 0 ? `${current.inRangePct}%` : '—'}
          </p>
        </div>
        <div className="text-center">
          <p className="text-sm font-bold text-indigo-600 dark:text-indigo-400">{current.readings}</p>
        </div>
        <div className="text-center">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400">
        ...
        {/* Difference row */}
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-gray-400 dark:text-gray-500">Diferencia</span>
        </div>
  ...
  return (
    <div className="card-hover bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
      <div className="flex items-start gap-5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h3 className="text-sm font-bold text-gray-800 dark:text-white">Puntaje de Salud</h3>
                ...
                <div key={day.date} className="flex-1 flex flex-col items-center gap-0.5">
                  <div
                    className={`w-full rounded-sm transition-all duration-300 ${
                      isToday ? 'bg-blue-500' : 'bg-blue-300 dark:bg-blue-600/60'
                    }`}
                    style={{ height: `${height}%` }}
                    title={`${day.label}: ${day.average} mg/dL`}
                  />
                  <span className={`text-[7px] font-medium ${isToday ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'}`}>
                    {day.label}
                  </span>
                </div>
              );
          ...
          <div className="grid grid-cols-3 gap-2 mt-3">
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2 text-center">
              <p className="text-[10px] text-gray-400 dark:text-gray-500">En rango</p>
              <p className="text-xs font-bold text-green-600 dark:text-green-400">
                {score.details.inRangePercentage}%
              </p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2 text-center">
              <p className="text-[10px] text-gray-400 dark:text-gray-500">Promedio</p>
              <p className="text-xs font-bold text-gray-700 dark:text-gray-200">
                {score.details.averageGlucose}
              </p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2 text-center">
              <p className="text-[10px] text-gray-400 dark:text-gray-500">Mediciones/día</p>
