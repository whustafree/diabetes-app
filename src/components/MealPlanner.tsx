    ...
    return (
      <div className="max-w-3xl mx-auto">
        <button
          ...
          className="flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-gray-700 mb-4 transition-colors"
        >
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="text-center">
        <div className="flex items-center justify-center gap-3 mb-2">
          ...
          <h2 className="text-2xl font-extrabold text-gray-800 dark:text-white">Plan de Comidas</h2>
        </div>
        <p className="text-gray-400 dark:text-gray-500 text-sm">Recetas saludables y balanceadas para diabéticos</p>
      </div>
          ...
          <div className="mt-4 space-y-3">
            <div className="flex gap-3">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 dark:text-white focus:bg-white dark:focus:bg-gray-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all text-sm"
              />
            </div>
            {glucoseSuggestion && (
              <div className="p-4 rounded-xl bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 space-y-3">
                <h4 className="font-bold text-gray-800 dark:text-white text-sm">{glucoseSuggestion.title}</h4>
                <p className="text-sm text-gray-600 dark:text-gray-300">{glucoseSuggestion.suggestion}</p>
                <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-3.5 rounded-2xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 dark:text-white focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all shadow-sm"
        />
        {searchQuery && recipeSuggestions.length > 0 && (
          <div className="absolute top-full mt-2 left-0 right-0 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 z-10 p-2 max-h-80 overflow-y-auto">
            {recipeSuggestions.map(recipe => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-800 dark:text-white truncate">{recipe.name}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500">{recipe.nutrition.calories} kcal · {recipe.prepTime} min</p>
                </div>
      ...
      <div className="flex items-center gap-2 justify-center">
        <button
          ...
          className="p-2 rounded-xl text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
        >
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all ${
                i === selectedDay
                  ...
                  : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-300 hover:text-gray-700 dark:hover:text-white border border-gray-100 dark:border-gray-600 hover:shadow-sm'
              }`}
            >
              {day.day}
            </button>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <div className="grid grid-cols-4 gap-3">
              ...
              <div className="text-center p-2 rounded-lg bg-orange-50">
                <div className="text-xs text-orange-600 font-semibold">Carbohidratos</div>
                <div className="text-sm font-bold text-orange-700">{currentDay.totalNutrition.carbs}g</div>
              </div>
              <div className="text-center p-2 rounded-lg bg-purple-50">
                <div className="text-xs text-purple-600 font-semibold">Grasas</div>
                <div className="text-sm font-bold text-purple-700">{currentDay.totalNutrition.fat}g</div>
              </div>
              <div className="text-center p-2 rounded-lg bg-green-50">
                <div className="text-xs text-green-600 font-semibold">Fibra</div>
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h4 className="font-bold text-gray-800 dark:text-white">{mealSlotLabels[meal.slot]}</h4>
                    ...
                    <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mt-0.5">{meal.recipe.name}</p>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-bold text-gray-800 dark:text-white">{meal.recipe.nutrition.calories}</div>
...
function RecipeCategory({ title, recipes, onClick }: { title: string; recipes: Recipe[]; onClick: (r: Recipe) => void }) {
  ...
  return (
    <div>
      ...
      >
        <h4 className="text-sm font-bold text-gray-600 dark:text-gray-400">{title} ({recipes.length})</h4>          {open ? <ChevronUp className="w-4 h-4 text-gray-400 dark:text-gray-500" /> : <ChevronDown className="w-4 h-4 text-gray-400 dark:text-gray-500" />}
      </button>
      {open && (
        <div className="grid sm:grid-cols-2 gap-2">
          {recipes.map(recipe => (
            <button
              ...
              className="text-left p-3 rounded-xl bg-gray-50 dark:bg-gray-700/50 hover:bg-blue-50 dark:hover:bg-blue-900/20 border border-gray-100 dark:border-gray-600 hover:border-blue-200 transition-all"
            >
              <p className="text-sm font-semibold text-gray-800 dark:text-white">{recipe.name}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1">
            <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-1">{recipe.name}</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">{recipe.description}</p>
            <div className="flex flex-wrap gap-2">
              {recipe.tags.map(tag => (
                <span key={tag} className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-600">
                  {tag}
                </span>
              ))}
        ...
        {/* Nutrition */}
        <div className="grid grid-cols-5 gap-3 p-4 rounded-xl bg-gray-50 dark:bg-gray-700/50">
          <div className="text-center">
            <Flame className="w-4 h-4 text-orange-500 mx-auto mb-1" />
            <div className="text-sm font-bold text-gray-800 dark:text-white">{recipe.nutrition.calories}</div>
            <div className="text-[10px] text-gray-400 dark:text-gray-500">kcal</div>
          </div>
          <div className="text-center">
            <Beef className="w-4 h-4 text-blue-500 mx-auto mb-1" />
            <div className="text-sm font-bold text-gray-800 dark:text-white">{recipe.nutrition.protein}g</div>
            <div className="text-[10px] text-gray-400 dark:text-gray-500">Proteína</div>
          </div>
          <div className="text-center">
            <Wheat className="w-4 h-4 text-orange-500 mx-auto mb-1" />
            <div className="text-sm font-bold text-gray-800 dark:text-white">{recipe.nutrition.carbs}g</div>
            <div className="text-[10px] text-gray-400 dark:text-gray-500">Carbs</div>
          </div>
          <div className="text-center">
            <Droplets className="w-4 h-4 text-purple-500 mx-auto mb-1" />
            <div className="text-sm font-bold text-gray-800 dark:text-white">{recipe.nutrition.fat}g</div>
            <div className="text-[10px] text-gray-400 dark:text-gray-500">Grasa</div>
          </div>
          <div className="text-center">
            <Salad className="w-4 h-4 text-green-500 mx-auto mb-1" />
            <div className="text-sm font-bold text-gray-800 dark:text-white">{recipe.nutrition.fiber}g</div>
            <div className="text-[10px] text-gray-400 dark:text-gray-500">Fibra</div>
          </div>
        </div>
        ...
        <ul className="space-y-2">
          {recipe.ingredients.map((ing, i) => (
            <li key={i} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
              ...
              <span className="text-sm font-semibold text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 px-3 py-1 rounded-lg">{ing.amount}</span>
            </li>
          ))}
        </ul>
      </div>
