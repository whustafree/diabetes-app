...
export default function ConfirmModal({
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-[scaleIn_0.2s_ease-out]"
      ...
      >
        <div className="flex items-start justify-between mb-4">
          ...
          {!loading && (
            <button
              onClick={onClose}
              className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 transition text-gray-400 flex-shrink-0"
            >
              <X className="w-5 h-5" />
            </button>
          )}
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            disabled={loading}
            className="flex-1 py-3 rounded-xl bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 font-semibold hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-50 transition-all"
          >
            {cancelLabel}
          </button>
          <button
