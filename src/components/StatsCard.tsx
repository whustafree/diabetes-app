    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {cards.map((card) => {
        ...
        return (
          <div
            key={card.label}
            className={`${card.bg} rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-all duration-200 hover:-translate-y-0.5`}
          >
            ...
            {card.sub && (
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 font-medium">{card.sub}</p>
            )}
          </div>
        );
      })}
